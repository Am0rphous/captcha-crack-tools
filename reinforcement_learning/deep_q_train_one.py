...
def window_slide(image, ans_V):
        ...
        for ii, x in enumerate(range(0, image.shape[1], stepSize)):
            for iii, y in enumerate(range(0, image.shape[0], stepSize)):
               ...
               idxAX += 1
        for i in range(16):
                          fps16.append(image[arrr[i][0]:arrr[i][1],arrr[i][2]:arrr[i][3],:])
...
def createNetwork():
    ...
    print (h_conv6.shape)
    h_conv3_flat = tf.reshape(h_conv6, [-1, 256])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    checkpoint = tf.train.get_checkpoint_state("saved_networks_one")
    ...
    while "flappy bird" != "angry bird":
         ...
         for step, key in enumerate(traindict.keys()):
             ...
             print ("NEW GAME", list([int(x) for x in resize_label]))
             #imgs(resize_fps16[0])
             ...
             for ix, ops in enumerate(resize_fps16):
                ...
                x_t1 = np.reshape(x_t1, (104, 104, 3))
                ...
                if t % 10000 == 0:
                    saver.save(sess, 'saved_networks_one/' + GAME + '-dqn', global_step = t)
