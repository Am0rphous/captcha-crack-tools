...
REPLAY_MEMORY = 2000 # number of previous transitions to remember \ количество предыдущих переходов для запоминания
BATCH = 16 # size of minibatch \ размер мини-партии 16#
FRAME_PER_ACTION = 1
"""
...
def readdata():
            with open("data/bicycles4x4/big_img.csv", 'r') as g:
                      ...
                      for igh, gh in enumerate(g[:]):
                          gh = gh.split('\n') 
                          gh = gh[0].split(';')
                          namefile = 'data/bicycles4x4/img/'+gh[0]+'.jpg'
                          #print namefile
...
def window_slide(image, ans_V):
        ...
        for ii, x in enumerate(range(0, image.shape[1], stepSize)):
            for iii, y in enumerate(range(0, image.shape[0], stepSize)):
               ...
               arrnp[x:x + w_width, y:y + w_height, :] = image[x:x + w_width, y:y + w_height, :]*.255
                      #arrnp[x:x + w_width, y:y + w_height, :] = image[x:x + w_width, y:y + w_height, :]
               #arrr.append((x , x + w_width, y, y + w_height ))
               fps16.append(arrnp)
                      #else: 
                      #image[x:x + w_width, y:y + w_height, :] = image[x:x + w_width, y:y + w_height, :]
                  #else:
                      #fps16.append(image)  
               idxAX += 1
                      #image[x:x + w_width, y:y + w_height, :] = image[x:x + w_width, y:y + w_height, :]*.255
        #print (arrr)
        #for i in range(16):
                  #if i == 1:
                          #arrnp = np.copy(image) #np.zeros(shape=(416,416,3), dtype=np.uint8)
                          #arrnp[arrr[i][0]:arrr[i][1],arrr[i][2]:arrr[i][3],:] = arrnp[arrr[i][0]:arrr[i][1],arrr[i][2]:arrr[i][3],:]*.255
                          #fps16.append(image[arrr[i][0]:arrr[i][1],arrr[i][2]:arrr[i][3],:]*.255)
                          #imgs(arrnp)
                  #else:                   
                          #fps16.append(arrnp)
                  #idxAY += 1
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    while "flappy bird" != "angry bird":
         ...
         for step, key in enumerate(traindict.keys()):
             ...
             print ("NEW GAME", list([int(x) for x in resize_label]), "FULL FRAME")
             #imgs(resize_fps16[0])
             ...
             for ix, ops in enumerate(resize_fps16):
                ...
                if t % FRAME_PER_ACTION == 0:
                    ...
                    a_t[0] = 1 # do nothing
                print ("AT NN", a_t, readout_t, "AT HUMAN", resize_label[ix])
                ...
                #print (ops.shape)
                x_t1 = ops #cv2.cvtColor(cv2.resize(np.array(ops, dtype=np.uint8),  (416, 416)), cv2.COLOR_BGR2GRAY)
                #imgs(x_t1)
                #ret, x_t1 = cv2.threshold(x_t1, 1, 255, cv2.THRESH_BINARY)
                #x_t1 = np.reshape(x_t1, (416, 416, 1))
                ...
                if t % 10000 == 0:
                    saver.save(sess, 'saved_networks_1/' + GAME + '-dqn', global_step = t)
