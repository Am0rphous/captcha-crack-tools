...
ACTIONS = 4 # number of valid actions \ количество действительных действий
GAMMA = 0.99 # decay rate of past observations \ скорость распада прошлых наблюдений
OBSERVE = 1000000. # timesteps to observe before training \ временные шаги, чтобы наблюдать перед тренировкой
...
def readdata():
            ...
            """
            FILE1 = open("data/taxi4x4/big_img.csv", 'r')
            FILE2 = open("data/bicycles4x4/big_img.csv", 'r')
            g1 = FILE1.readlines()
            g2 = FILE2.readlines()
...
def get_batch(k):
                     ...
                     return resize_image, resize_label, clas
...
def gameaction(y):
        ...
        return reward, terminal
"""
def gameaction(y,x,z, readout_t ):
        #print (y,x,z)
        reward = 0.1
        terminal = False
        if z == 'taxi4x4':
                if y[1] == x:
                    reward = 1
                    print (readout_t, "GOOOD SCORE # TAXI # gameaction NN:", y, "----  MY:", x)
                else:
                    print (readout_t, "BAD SCORE # TAXI # gameaction NN:", y, "----  MY:", x)
                    terminal = True
                    reward = -1
        else:
                if y[1] == x:
                    reward = 1#0.5
                    print (readout_t, "GOOOD SCORE # BIKE # gameaction NN:", y, "----  MY:", x)
                else:
                    print (readout_t, "BAD SCORE # BIKE # gameaction NN:", y, "----  MY:", x)
                    terminal = True
                    reward = -1#-0.5
        """
        #    if y[3] == 1:
...
def get_loss(y_, y):
    ...
    print ("GET LOSS")
    digits_loss = tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=y_ ) # 3xinput
    digits_loss = tf.reshape(digits_loss, [-1, 2])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    print (readout[:,:2].shape)
    # s(input), readout(tf.matmul), h_fc1(tf.nn.relu)
    a = tf.placeholder("float", [None, 2])
    #c = tf.placeholder("float", [None, ACTIONS])
    y = tf.placeholder("float", [None])
    C = tf.placeholder("float", [None, 2])
    readout_action = tf.reduce_sum(tf.multiply(readout[:,:2], a), reduction_indices=1) #действие считывания - поэлементно умножаем readout и a - сумма по оси 1
    #readout_class = tf.reduce_sum(tf.multiply(readout, c), reduction_indices=1)
    cost = tf.reduce_mean(tf.square(y[0] - readout_action))
    
    cost_class = get_loss(C, readout[:,2:])
    cost += cost_class
    #cost_c = tf.reduce_mean(tf.square(y - readout_class))
    #cost = cost_a + cost_c
    ...
    #do_nothing[0] = 1
    ####
    r_image, r_label, r_class = get_batch(traindict.keys()[0])
    ...
    checkpoint = tf.train.get_checkpoint_state("saved_networks_two3")
    ...
    while "flappy bird" != "angry bird":
         ...
         for step, key in enumerate(traindict.keys()):
             resize_image, resize_label, resize_class = get_batch(key)
             resize_class_arr = getArrClass(resize_class)

             resize_fps16 = window_slide(resize_image, resize_label)

             print ("NEW GAME", list([int(x) for x in resize_label]), resize_class_arr)
             #imgs(resize_fps16[0])
             ...
             for ix, ops in enumerate(resize_fps16):
                ...
                #imgs(ops)  
                a_t = np.zeros([2])
                #a_t_class = np.zeros([2])
                #a_c = np.zeros([ACTIONS])
                #a_t = np.zeros([2])
                #a_c = np.zeros([2])
                ...
                if t % FRAME_PER_ACTION == 0:
                    if random.random() <= epsilon:
                        ...
                        #class_index = random.randrange(ACTIONS)
                        a_t[random.randrange(2)] = 1
                        #a_t_class[random.randrange(2)] = 1
                        #a_c[random.randrange(2)] = 1
                        #s = random.randrange(0, 2)
                    else:
                        action_index = np.argmax(readout_t[:2])
                        #best = tf.argmax(tf.reshape(y[:, 1:], [-1, 8, len(CHARS)]), 2) # сдесь я извлехая лучшее из матрици
                        #correct = tf.argmax(tf.reshape(y_[:, 1:], [-1, 8, len(CHARS)]), 2)
                        #class_index = np.argmax(readout_t[2:], [2, 2])
                        #print ("----------Action----------", action_index, "<<<<>>>>", readout_t)
                        #a_t[action_index] = 1
                        #a_t[class_index] = 1
                        a_t[action_index] = 1
                        #a_t_class[action_index] = 1
                        #a_t = np.concatenate((a_t_a,a_t_c), axis=None)
                ...
                r_t, terminal = gameaction(a_t, resize_label[ix], resize_class, readout_t)
                #if ix>0:
                #        r_t, terminal = gameaction2(a_t, resize_label[ix], D[ix-1][1], resize_label[ix-1]) 
                #else:
                #        r_t, terminal = gameaction(a_t, resize_label[ix])
                        
                D.append((s_t, a_t, r_t, s_t1, resize_class_arr, terminal))
                ...
                if t > OBSERVE:
                    ...
                    s_CLASS_batch = [d[4] for d in minibatch]
                ...
                if t % 10000 == 0:
                    saver.save(sess, 'saved_networks_two3/' + GAME + '-dqn', global_step = t)
...
"""
Добавил дополнительный класс - велосипед
выход из сети 1x4([0,0,0,0]) - правильный ответ для такси = [0,1,0,1]
Первый срез [:2] = [0,0] выбор квадратика, 
Второй срез [1:] = [0,0] класс: такси или "не такси"
Переделал алгоритм обучения:
Если сеть отгадала квадрат и класс - вознаграждение 1, неправильный ответ -1,
Сеть отгадала квадрат и отнесла его к "не такси" - вознаграждение 1, неправильный ответ 0.1
Сравниваю 16 ответов с ответами работников = правильно +10, неправильно -10
Тестирую дополнительную корекцию каждые 10 000 шагов (Оператор: человек)
если мой ответ совпадет с ответом сети 100, не совпадет -100

Первые попытка дали результат:
74 для целой картинки и выделенного квадрата,
76 для квадрата из картинки

Ищу балан при котором сеть будет мотивирована обучаться - какой суммой лучше наказывать какой поощрять 
