...
def g(img, tp):
                imzero = np.zeros(shape=(416, 416, 3), dtype=np.uint8)
                img_resized0 = cv2.resize(img, (212, 212)) #img.resize(size=(size, size))
                imzero[100:312, 100:312, :] = img_resized0
                img_resized = np.reshape(imzero, [1, 416, 416, 3])
                I, B, C = sess.run([inputs, boxes, cl_de], feed_dict={inputs: img_resized})
                boxeS, scores, labels = cpu_nms(B, C, len(classes), max_boxes=3000, score_thresh=0.1, iou_thresh=0.2) #Bike
                result, dtimg, dt = draw_boxes(imzero, boxeS, scores, labels, classes, 416, tp)
                #fff, dt2 = imcr(np.array(dt), 3)
                #retval, image = cv2.imencode('.jpg', np.array(dtimg))
                #return fff, image, labels, boxeS
                return result, dtimg
...
class record():
     def __init__(self):
         self.file = open('not_hydrants.txt', 'w')
         print "LOAD"
     def save(self, i):
         #print i
         self.file.write(i+"\n") 
...
if __name__ == "__main__":
   ...
   for ix, u in enumerate(list(FL[0].keys())):
       ...
       if PP[0] != []:
           print PP[0]
           R.save(FL[0][u][0]+":"+str(PP[0]))
           #imgs(PP[-1])

"""
Папка нет гидрантов
1) Картинки порезаны я не смог их воссоздать
как полноценное изображение для моего алгоритма
2) использовал алгоритм как классификатор
   а) сделал ресайз
   б) сделал пустое изображение черного и на него наложил изображение с повышенным разрешением 215х215
3) Понизил коэффициент срабатывапния -
вместе с гидрантами были отмечены изображения похожие еа нидранты и так же ошибки сети

Папка есть гидранты -
точность от 70 до 93 - маленькое разрешение и маленькие обьекты

пути решения проблемы использовать
