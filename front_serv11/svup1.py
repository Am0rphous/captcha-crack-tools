...
class MainUpload(tornado.web.RequestHandler):
    def get(self):
        ...
        for x in mgDC:
           ...
           items[x] = 0#len(J)
        self.render("upload2.html", title="Нейронная сеть/Тренировка", items=json.dumps(items))
    ...
    def post(self):
        ...
        if int(data_json["type"]) != 4:
                ...
                if data_json["st"] == "prep":
                        ...
                        #print ("END") 
                        kolo = {"arch": 'archiv/traffic_lights4x4.tar.gz', "csv": data_json["task"]+'.csv', "type": 4}
                        self.write(kolo)
