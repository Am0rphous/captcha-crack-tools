...
def LeoPost(data, idx, task):
    url = "http://144.76.16.247:8084/"#"http://148.251.0.181:8084/"
...
def thread_function(new, data, task):
   #R = record(idx)
   for ix, u in enumerate(data):
          ...
          #print ("Leopost", posttoCH, [int(x) for x in PP["text"]], u["_id"])
          #posttoCH["answ_L"] = [int(x) for x in PP["text"]]
          u["answ_L"] = [int(x) for x in PP["text"]]
          u["_id"] = ObjectId(u["_id"])
          new.upd_post(u["_id"], u)
          #new.append(u)
          #print ("Leopost", posttoCH, "=======", u,"END")#[int(x) for x in PP["text"]], u["_id"])
          #R.save(u+";"+str(PP))
